...
export function renderUserList() {
  ...
  for (const id of idsOrdenados) {
    ...
    // Fecha de vencimiento corta para el header
    let vencCorto = "—";
    if (fv) {
...
export function switchAtletaTab(id, tab) {
  ...
  // Si abre pagos, setea editingUserId por las dudas
...
export function seleccionarTipoPagoAtleta(id, tipo) {
  ...
  // Calcular nuevo vencimiento automáticamente
  const u = cacheUsers[id];
  if (!u) return;
